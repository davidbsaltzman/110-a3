...
RemovePathname(struct unixfilesystem *fs, char *pathname)
{
  ...
  char *endOfDirNamePtr = strrchr(pathname, '/');
  char directoryName[strlen(pathname)];
  strncpy(directoryName, pathname, endOfDirNamePtr - pathname);
  directoryName[endOfDirNamePtr - pathname] = '\0';
  
  int directoryInumber = pathname_lookup(fs, directoryName);
  
  printf("Before removing %s:\n", pathname);
  PrintDirectory(fs, directoryName); // print directory entries before removing file
  
  int inumber = pathname_lookup(fs, pathname);
  
  remove_file(fs, inumber);
  
  fs->superblock.s_inode[fs->superblock.s_ninode] = inumber;
  fs->superblock.s_ninode++;
  diskimg_writesector(fs->dfd, SUPERBLOCK_SECTOR, &fs->superblock);
  
  printf("After removing %s:\n", pathname);
  PrintDirectory(fs, directoryName); // print directory entries after removing file
}
...
GetDirEntries(struct unixfilesystem *fs, int inumber, struct direntv6 *entries, int maxNumEntries)
{
  ...
  for (int bno = 0; bno < numBlocks; bno++) {
    ...
    for (i = 0; i <  numEntriesInBlock ; i++) {
      char allocated = 1;
      for (int j = 0; j < fs->superblock.s_ninode; j++)
      {
        if (fs->superblock.s_inode[j] == dir[i].d_inumber)
        {
          allocated = 0;
          break;
        }
